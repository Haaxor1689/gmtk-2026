...
func try_move(node: GridNode, direction: Vector2, is_push: bool = false) -> Dictionary:
	var new_position := node.grid_pos + direction

	for t in tilemaps:
		if t.is_collidable:
			if t.get_cell_source_id(new_position) != -1:
				return {"success": false, "cost": 1.0 * leak_modifier}

	for obj in objects:
		if obj.grid_pos == new_position:
			if obj.is_pushable and !is_push and Global.try_move(obj, direction, true)["success"]:
				print(obj.name)
				return {"success": true, "cost": obj.push_cost * leak_modifier}
			else:
				return {"success": false, "cost": 1.0 * leak_modifier}
	...
	return {"success": true, "cost": 1.0 * leak_modifier}
